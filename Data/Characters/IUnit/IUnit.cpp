...
IUnit::IUnit(const int s_hp, const float s_armor, const float s_avoidChance, const int s_damage,
    const float s_critChance, const string& s_name) noexcept:
    m_hp {s_hp},
    m_armor {s_armor},
...
IUnit::IUnit(const Parameters& s_params) noexcept: m_hp{s_params.m_hp}, m_armor{s_params.m_armor},
...
*/
void IUnit::hit(IUnit* const s_target) const noexcept{
...
*/
void IUnit::takeDamage(int s_damage) noexcept {
    m_hp -= s_damage;
    if(m_hp < 0)
        m_hp = 0;
}
